...
class Enemy(PhysicsEntity):
    def __init__(self, game, color, pos, size):
        super().__init__(game, 'enemy/' + color, pos, size, anim_offset=(-3, -2))
        ...
        if self.walking:
            if tilemap.solid_check((self.rect().centerx + (-7 if self.flip else 7), self.pos[1] + 23)):
                if self.collisions['right'] or self.collisions['left']:
                    ...
                    if self.can_see_player(tilemap) and random.random() * 300 >= 299:
                        self.shoot()
            else:
                self.flip = not self.flip
            self.walking = max(0, self.walking - 1)
            if not self.walking and self.can_see_player(tilemap):
                self.shoot()
